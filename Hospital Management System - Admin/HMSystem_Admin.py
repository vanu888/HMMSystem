...
# Sample data to hold patients records
patients = {}
...
os.system(f"attrib +h {data_file}")  # Hide data file
os.system(f"attrib +h {password_file}")  # Hide password file
...
# Paths to png files
logo = resource_path("Hospital Management System - Admin/icons/logo.png")
welcome = resource_path("Hospital Management System - Admin/icons/welcome.png")
logo_jpg = resource_path("Hospital Management System - Admin/icons/logo.jpg")
login = resource_path("Hospital Management System - Admin/icons/login.jpg")
pdf = resource_path("Hospital Management System - Admin/icons/pdf.png")


def open_user_manual():
    url = "https://vihanga-anuththara.github.io/HMMSystem/#chapter3"  # User Manual page link
    webbrowser.open(url)


# Create main application
class HospitalManagementAdminApp:
    def __init__(self, root):
        self.root = root  # Main window name as root
        self.root.title("Hospital Management System - Admin")  # Main title
        ...
        self.root.geometry(f"{800}x{600}+{int(x)}+{int(y)}")
        ...
        # Open pdf png
        self.pdf_image = Image.open(pdf)
        self.pdf_image = self.pdf_image.resize((20, 20), Image.LANCZOS)
        self.pdf_photo = ImageTk.PhotoImage(self.pdf_image)
        ...
        self.style.configure(
            "TNotebook.Tab", padding=[15, 8], font=("Arial", 12, "bold")
        )
    ...
    def show_welcome_screen(self):
        ...
        self.welcome_window.title("Hospital Management System - Admin")
        ...
        x = (screen_width / 2) - (800 / 2)
        y = (screen_height / 2) - (600 / 2)
        self.welcome_window.geometry(f"{800}x{600}+{int(x)}+{int(y)}")
        self.welcome_window.configure(bg="#f0f0f0")  # Set background color
        icon = ImageTk.PhotoImage(file=logo)
        self.welcome_window.iconphoto(False, icon)

        # Show Intro details
        Intro_label = ttk.Label(
            self.welcome_window,
            text="Hospital Management System - Admin",
            background="#f0f0f0",
            font=("Arial", 14, "bold"),
        )
        ...
        self.welcome_image = Image.open(welcome)
        ...
        welcome_label_image = tk.Label(
            self.welcome_window, image=self.welcome_photo, background="#f0f0f0"
        )
        welcome_label_image.pack(pady=10)

        # Create a loading bar
        self.loading_bar = ttk.Progressbar(
            self.welcome_window, orient="horizontal", length=400, mode="indeterminate"
        )
        ...
        loading_label = ttk.Label(
            self.welcome_window, text="Loading, please wait...", background="#f0f0f0"
        )
    ...
    def create_login_tab(self):
        ...
        self.change_password_button = ttk.Button(
            self.login_frame, text="Change Password", command=self.change_password
        )
        self.change_password_button.pack(pady=10)

        # Display an image
        self.login_image = Image.open(login)
        self.login_image = self.login_image.resize(
            (200, 200), Image.LANCZOS
        )  # Resize image
        self.login_photo = ImageTk.PhotoImage(self.login_image)
        login_label_image = tk.Label(
            self.login_frame, image=self.login_photo, background="#f0f0f0"
        )
        login_label_image.pack(pady=50)
    ...
    def change_password(self):
        ...
        self.change_password_window.geometry(f"{300}x{200}+{int(x)}+{int(y)}")
        icon = ImageTk.PhotoImage(file=logo)
        self.change_password_window.iconphoto(False, icon)

        # Old Password Entry
        old_password_label = ttk.Label(
            self.change_password_window, text="Old Password:"
        )
        ...
        new_password_label = ttk.Label(
            self.change_password_window, text="New Password:"
        )
        ...
        submit_button = ttk.Button(
            self.change_password_window, text="Submit", command=self.submit_new_password
        )
        ...
        if os.path.exists(password_file):
            with open(password_file, "r") as f:
                ...
                current_password = data.get(
                    "password", "password"
                )  # Default password if not  ( use as password )
    ...
    def submit_new_password(self):
        ...
        if old_password != current_password:
            ...
            if new_password:
                ...
                )  # Show error message and load the current password from the file
    ...
    def create_display_tab(self):
        ...
        self.refresh_button = ttk.Button(
            self.display_frame, text="Refresh Data", command=self.refresh_data
        )
        ...
        self.export_button = ttk.Button(
            ...
            image=self.pdf_photo,
            compound="left",
            command=self.export_all_data_to_pdf,
        )
    ...
    def refresh_data(self):
        ...
        for patient_id, info in patients.items():
            self.tree.insert(
                "",
                "end",
                values=(patient_id, info["name"], info["age"], info["description"]),
            )
    ...
    def export_all_data_to_pdf(self):
        if not patients:
            ...
            )  # Show error message ,when no data to export
            return

        pdf_file_name = (
            "Patients_data_by_HMSystem_Admin.pdf"  # Set file name to the pdf file
        )
        ...
        for patient_id, info in patients.items():
            c.drawString(
                100,
                y_position,
                f"ID: {patient_id}, Name: {info['name']}, Age: {info['age']}, Description: {info['description']}",
            )
        ...
        messagebox.showinfo(
            "Success", f"Exported all data to {pdf_file_name}."
        )  # Show a message after the save pdf file
    ...
    def create_updateStatus_tab(self):
        ...
        # Get patient ID
        self.updateStatus_id_label = ttk.Label(
            self.updateStatus_frame, text="Patient ID: "
        )
        ...
        self.updateStatus_description_label = ttk.Label(
            self.updateStatus_frame, text="Update Description:"
        )
        ...
        self.updateStatus_button = ttk.Button(
            self.updateStatus_frame, text="Update Status", command=self.update_status
        )
    ...
    def update_status(self):
        patient_id = (
            self.updateStatus_id_entry.get()
        )  # Get input from updateStatus Tab to Patient ID
        status = (
            self.updateStatus_description_entry.get()
        )  # Get input from updateStatus Tab to update status

        if patient_id in patients:  # Check patient_id in the dataset
            patients[patient_id]["description"] = status
            messagebox.showinfo(
                "Success",
                f"Updated {patients[patient_id]['name']} (ID: {patient_id}) with new status.",
            )  # Show a message after the successfully updated data
    ...
    def create_update_tab(self):
        ...
        self.add_button = ttk.Button(
            self.update_frame, text="Add Data", command=self.add_data
        )
        ...
        self.delete_button = ttk.Button(
            self.update_frame, text="Delete Data", command=self.delete_data
        )
    ...
    def add_data(self):
        ...
        if patient_id and name and age.isdigit() and self.is_valid_age(age):
            if patient_id in patients:
                ...
                messagebox.showinfo(
                    "Success", f"Added {name} with ID {patient_id} and age {age}."
                )
    ...
    def update_data(self):
        ...
        if patient_id in patients:
            ...
            if name and age.isdigit() and self.is_valid_age(age):
                ...
                messagebox.showinfo(
                    "Success",
                    f"Updated {patient_id} to {name} with age {age} and {description}.",
                )
                ...
                messagebox.showerror(
                    "Error", "Please enter valid name, age and description."
                )
        else:
    ...
    def delete_data(self):
        ...
        if patient_id in patients:
            ...
            messagebox.showinfo(
                "Success", f"Deleted data with ID {patient_id}."
            )  # Show a message after deleted the data
            ...
            )  # Show an error message ,when patient id not founded
    ...
    def create_check_tab(self):
        ...
        self.check_label = ttk.Label(
            self.check_frame, text="Enter Patient ID to Check:"
        )
        ...
        self.check_button = ttk.Button(
            self.check_frame, text="Check Patient", command=self.check_data
        )
        ...
        self.search_button = ttk.Button(
            self.check_frame, text="Search Patient Data", command=self.search_data
        )
    ...
    def check_data(self):
        ...
        if patient_id in patients:
            ...
            messagebox.showinfo(
                "Patient Found",
                f"ID: {patient_id}, Name: {patient_info['name']}, Age: {patient_info['age']}.",
            )
        else:
    ...
    def create_about_tab(self):
        ...
        # Display an image
        self.about_image = Image.open(welcome)
        self.about_image = self.about_image.resize(
            (400, 300), Image.LANCZOS
        )  # Resize image
        self.about_photo = ImageTk.PhotoImage(self.about_image)
        about_label_image = tk.Label(
            self.about_frame, image=self.about_photo, background="#f0f0f0"
        )
        about_label_image.pack(pady=10)

        # Create a button to opens the user manual link
        user_manual_button = tk.Button(
            self.about_frame, text="User Manual", command=open_user_manual
        )
        user_manual_button.pack(pady=20)


if __name__ == "__main__":
    root = tk.Tk()
    root.withdraw()  # Hide the main window initially
    app = HospitalManagementAdminApp(root)
    root.mainloop()
