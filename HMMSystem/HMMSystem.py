...
# Paths to icons
logo = resource_path("HMMSystem/icons/logo.png")
welcome = resource_path("HMMSystem/icons/welcome.png")
logo_jpg = resource_path("HMMSystem/icons/logo.jpg")
login = resource_path("HMMSystem/icons/login.jpg")
pdf = resource_path("HMMSystem/icons/pdf.png")
...
class HospitalMedicineManagementApp:
    ...
    def refresh_data(self):
        ...
        for med_id, info in medicines.items():
            self.tree.insert(
                "",
                "end",
                values=(med_id, info["name"], info["quantity"], info["supplier"]),
            )
    ...
    def export_all_data_to_pdf(self):
        ...
        pdf_file_name = (
            "Medcines data by HMMSystem.pdf"  # Create a variable to assign pdf name
        )
        ...
        for med_id, info in medicines.items():
            c.drawString(
                100,
                y_position,
                f"ID: {med_id}, Name: {info['name']}, Quantity: {info['quantity']}, Supplier: {info['supplier']}",
            )
    ...
    def create_issue_tab(self):
        ...
        self.issue_quantity_label = ttk.Label(
            self.issue_frame, text="Quantity to issue: "
        )
        ...
        self.issue_button = ttk.Button(
            self.issue_frame, text="Issue medicines", command=self.issue_medicines
        )
        self.issue_button.pack(pady=10)

    # Function for Issue medicines
    def issue_medicines(self):
        ...
        if med_id in medicines and quantity_to_issue.isdigit():
            ...
            if quantity_to_issue > current_quantity:
                messagebox.showerror(
                    "Warning", "Not enough quantity available to issue!"
                )
            else:
                medicines[med_id]["quantity"] -= quantity_to_issue  # Issue the quantity
                messagebox.showinfo(
                    "Success",
                    f"Issued {quantity_to_issue} of {medicines [med_id]['name']} (ID: {med_id})",
                )
    ...
    def create_medicine_tab(self):
        ...
        self.add_button = ttk.Button(
            self.medicine_frame, text="Add Medicine", command=self.add_medicine
        )
        self.add_button.pack(pady=10)

        self.update_button = ttk.Button(
            self.medicine_frame, text="Update Medicine", command=self.update_medicine
        )
        self.update_button.pack(pady=10)

        self.delete_button = ttk.Button(
            self.medicine_frame, text="Delete Medicine", command=self.delete_medicine
        )
    ...
    def add_medicine(self):
        ...
        if med_id and name and quantity.isdigit():
            if med_id in medicines:
                ...
                messagebox.showinfo(
                    "Success", f"Added {name} with ID {med_id} and quantity {quantity}."
                )
            ...
            messagebox.showerror(
                "Error", "Please enter valid ID, name, quantity and a supplier."
            )
    ...
    def update_medicine(self):
        ...
        if med_id in medicines:
            ...
            if name and quantity.isdigit():
                ...
                messagebox.showinfo(
                    "Success",
                    f"Updated {med_id} to {name} with quantity {quantity} and {supplier}.",
                )
                ...
                messagebox.showerror(
                    "Error", "Please enter valid name, quantity and supplier."
                )
        else:
    ...
    def create_check_tab(self):
        ...
        self.check_label = ttk.Label(
            self.check_frame, text="Enter Medicine ID to Check:"
        )
        ...
        self.check_button = ttk.Button(
            self.check_frame, text="Check Medicine", command=self.check_medicine
        )
        ...
        self.search_button = ttk.Button(
            self.check_frame, text="Search Medicine", command=self.search_medicine
        )
    ...
    def check_medicine(self):
        ...
        if med_id in medicines:
            ...
            messagebox.showinfo(
                "Medicine Found",
                f"ID: {med_id}, Name: {med_info['name']}, Quantity: {med_info['quantity']}.",
            )
        else:
            messagebox.showerror("Error", "Medicine ID not found.")
    ...
    def create_chart_tab(self):
        ...
        self.plot_button = ttk.Button(
            self.chart_frame,
            text="Graph Medicines Data",
            command=self.plot_medicine_data,
        )
    ...
    def plot_medicine_data(self):
        ...
        filtered_medicines = {
            med_id: info
            for med_id, info in medicines.items()
            if info["supplier"] == supplier_name
        }
        ...
        quantities = [
            info["quantity"] for info in filtered_medicines.values()
        ]  # Take quantities
